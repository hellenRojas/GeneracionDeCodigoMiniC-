...
class AContextual : Parser1BaseVisitor<Object>
{
    ...
    public int nivGloblal;
    public string varAct;
    ...
    public override object VisitVarDeclAST([NotNull] Parser1.VarDeclASTContext context)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                for (int i = 0; i <= context.ID().Length - 1; i++)
                {
                    if (table.buscarInsert(context.ID(i).GetText(),nivelActual) == null)
                    {
    ...
    public override object VisitDesignatorStatAST([NotNull] Parser1.DesignatorStatASTContext context)
    {
        try
        {
            ...
            {
                nivGloblal = 0;
                string tipoID1 = (string)Visit(context.designator());
                nivGloblal = 1;
                string tipoID2 = (string)Visit(context.designator());
                ...
                nivGloblal = 0;
                string tipoExpre1 = (string)Visit(context.expr());
                nivGloblal = 1;
                string tipoExpre2 = (string)Visit(context.expr());

                if (tipoID1 == null && tipoID2==null)
                {
                    throw new Exception("Linea: " + lineaActual + "-> Variable " + varAct+ " no definida");
                }
                if (tipoExpre1 == null && tipoExpre2 == null)
                {
                    throw new Exception("Linea: " + lineaActual + "-> Variable " + varAct + " no definida");
                }


                if (tipoExpre2 != null && tipoExpre1 == null)
                {
                    if ((tipoExpre2 == "int[]" || tipoExpre2 == "char[]") && esNew == false)
                    {
                        throw new Exception("Linea: " + context.ASIGN().Symbol.Line + "-> " + "Error no se pueden asignar arreglos directamente");
                    }
                }
                else if (tipoExpre2 == null && tipoExpre1 != null)
                {
                    if ((tipoExpre1 == "int[]" || tipoExpre1 == "char[]") && esNew == false)
                    {
                        throw new Exception("Linea: " + context.ASIGN().Symbol.Line + "-> " + "Error no se pueden asignar arreglos directamente");
                    }
                }
                ...
                if ((tipoExpre1 != tipoID1) && (tipoExpre2 != tipoID2 )&& (tipoExpre1 != tipoID2) && (tipoExpre2 != tipoID1) && (tipoExpre1 != "null") && (tipoExpre2 != "null"))
                {
                    throw new Exception("Linea: " + context.ASIGN().Symbol.Line + "-> " + "Tipos Incompatibles ( " + tipoID2 + "," + tipoExpre2 + ")");
                }
    ...
    public override object VisitDesignatorAST([NotNull] Parser1.DesignatorASTContext context)
    {
        ...
        if (context.ID(1) != null && table.buscarPNivel(context.ID(0).GetText(), nivGloblal) != null)
        {
            string  tipo = table.buscarPNivel(context.ID(0).GetText(),nivGloblal).tipo;
        ...
        else
        {
            ...
            */
            ...
            {
                varAct = context.ID(0).ToString();
                lineaActual = context.ID(0).Symbol.Line;
                return null;
            }
        }
    }
